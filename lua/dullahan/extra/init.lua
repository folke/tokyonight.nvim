...
function M.setup()
  M.docs()
  local dullahan = require("dullahan")
  ...
  for extra, info in pairs(M.extras) do
    package.loaded["dullahan.extra." .. extra] = nil
    local plugin = require("dullahan.extra." .. extra)
    for style, style_name in pairs(styles) do
      dullahan.setup({ style = style })
      dullahan.load({ style = style })
      vim.cmd.colorscheme("dullahan-" .. style)
      local colors = require("dullahan.colors").setup({ transform = true })
      local fname = extra .. "/tokyonight_" .. style .. "." .. info.ext
      colors["_upstream_url"] = "https://github.com/folke/dullahan.nvim/raw/main/extras/" .. fname
